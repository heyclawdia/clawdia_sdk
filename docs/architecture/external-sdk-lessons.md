...
| OpenAI Codex tools | [repo](https://github.com/openai/codex), [apply patch guide](https://developers.openai.com/api/docs/guides/tools-apply-patch), [apply-patch source](https://github.com/openai/codex/blob/main/codex-rs/apply-patch/src/lib.rs), [view-image source](https://github.com/openai/codex/blob/main/codex-rs/core/src/tools/handlers/view_image.rs) | 2026-05-24 | Small model-facing tool surface, structured patch application, typed image viewing, explicit tool dispatch, hooks, telemetry, and sandbox-aware filesystem reads. | Codex product prompts, product shell policy, and hosted tool routing as SDK defaults. | Keep workspace tools simple while implementing typed read pipelines, structured future patching, policy refs, effect records, and parser metadata. |
...
## Workspace Reader Completion Source-Audit Note

Date: 2026-05-24

The workspace-reader completion pass rechecked the oh-my-pi and Codex tool lessons already recorded in the source audit. The accepted implementation lesson is that a single model-facing `workspace_read` surface can stay simple while routing through typed internal readers for text, PDFs, media, archives, SQLite, and URI/resource-like inputs. The SDK-specific constraint remains stricter than product tools: outputs are bounded, stored behind content refs during tool execution, parser metadata is explicit, and ambient network, platform OCR, Photos-library access, or proprietary RAW conversion fail closed unless a host adapter and policy are attached.
...
| OpenAI Codex tools | Small tool surface, structured patching, typed image viewing, sandbox-aware file reads, and tool dispatch with hooks/telemetry. | Product-specific prompts, CLI UX, and hosted runtime assumptions stay outside SDK defaults. |
