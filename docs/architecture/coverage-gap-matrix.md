...
This file is a Phase 1 coverage audit, not an implementation-ready contract. Current coding handoff authority lives in [../contracts/README.md](../contracts/README.md) and [../workstreams/README.md](../workstreams/README.md). Historical plans under `docs/reference/plans/` are background only.

## Goal Acceptance Coverage

| Acceptance item | Covered in | Status |
| --- | --- | --- |
| Markdown documentation only. | This doc set and risk note. | Covered. |
| No code, Rust source files, executable tests, or fixtures. | All Rust is fenced conceptual sketch text. | Covered. |
| Demanding host scenarios studied. | [../examples/README.md](../examples/README.md), external SDK lessons, and the risk note. | Covered as product-neutral scenario coverage, not SDK ownership. |
...
| Caveats, risks, tradeoffs, migration. | [architecture-proposal.md](architecture-proposal.md) and risk note. | Covered. |
| Stop for user review before deeper design. | Phase 1 ends after these docs. | Covered. |

## Generic Host Scenario Coverage

| Scenario | Proposed support | Gaps before implementation |
| --- | --- | --- |
| Stateful external runtime path. | `ExternalAgentAdapter` plus provider-native runtime behind host dispatch boundaries. | Need adapter test contracts and migration plan. |
...
| Memory prompt injection and memory tools. | `MemorySystem` port and context item lineage. | Need memory capability schema and privacy policy. |
...
| --- | --- | --- |
| Strands | Deep loop, hooks, sessions, metadata, streaming, tools, agent-as-tool, multi-agent, bidi, OTel. | Phase 2 should turn chosen loop states into tests. |
...
The later-doc arrangement has already been created. Active implementation authority is now:
...
Historical plans under `docs/reference/plans/` are background only.

## Remaining Design Gaps Carried Into The Phase 2 Handoff
...
- Extension SDK packaging needs smoke tests to avoid stale packaged fallback resources.
- OTel GenAI conventions may change; SDK-specific namespacing must absorb drift.
- Migration from any existing product services needs staged compatibility plans.
