...
| [agent-pool-store-coordination.md](agent-pool-store-coordination.md) | Multi-process `AgentPoolStore` coordination with SQLite as a toolkit adapter and MCP/RPC left as adapter choices. |
...
| Multi-process agent coordination | [agent-pool-store-coordination.md](agent-pool-store-coordination.md) | `AgentPool`, `AgentPoolStore`, `RunMessage`, `WakeCondition`, `AgentPoolStoreCursor`, `ContentRef` | concrete store deployment, RPC/MCP/network transport, process lifecycle, UI reactions | agent-pool/message/wake events and journals remain source of truth; store records pool-scoped membership, messages, wakes, dedupe, and watch cursors | two handles see shared members/messages/wakes, missing target fails closed, subscriptions stay pool-scoped |
