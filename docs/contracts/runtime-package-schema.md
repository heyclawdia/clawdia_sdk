...
| `AgentAsTool` | [07 Subagents](../workstreams/_roles/07-subagents-coordination.md) | subagent capability sidecar lowering to `SubagentRequest` and a child `RunRequest` | parent package fingerprint lineage, child agent ID/version, route policy, selected provider route, context handoff variant/policy refs, retained capability IDs, recursive-strip manifest, mailbox policy, lifecycle refs, isolation refs, redaction/content-capture/retention refs | `SubagentStarted`, `SubagentHandoff`, `SubagentEvent`, terminal subagent kinds, usage rollup | child start effect intent/result, subagent records, child lifecycle records, usage rollup | child package stripping fixture, handoff policy matrix, parent cancel/detach tests, mailbox/clarification fixtures, event wrapping fixture |
...
- Subagent IDs, parent package fingerprint lineage, child route policy, selected provider route, context handoff policy refs, child tool policy, retained capability IDs, recursive-strip manifest, mailbox policy, lifecycle/detach refs, isolation refs, and redaction/content-capture/retention refs.
