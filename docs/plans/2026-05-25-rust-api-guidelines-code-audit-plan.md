...
- Follow-up decision: because the SDK is still alpha, large public result errors were fixed structurally where the error itself should stay cheap. `ContentResolutionError` now boxes its large content-ref payload, and structured-output validation returns boxed `ValidationErrorReport` failures so public `Result` APIs stay inexpensive without removing the rich report type.
- The remaining Clippy findings are intentional API-shape decisions, not hidden backlog. Durable serde payload enums (`JournalRecordPayload`, `StructuredOutputRecord`, `OutputDeliveryRecord`, `ValidatedOutputPublicationStep`, and `AgentPoolStoreRecordPayload`) keep direct variants for pattern-match and fixture ergonomics until a dedicated envelope migration. Wide lineage/record constructors keep their explicit fields until a builder/command-object API pass. Each accepted lint is now a local `#[expect(..., reason = "...")]`.
- The full workspace Clippy gate now passes: `cargo clippy --workspace --all-targets -- -D warnings`.
